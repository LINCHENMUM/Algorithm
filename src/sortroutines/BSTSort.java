...
public class BSTSort extends Sorter {
	...
	public static void main(String[] args) {
		...
		int[] testInt={1, 4, 2, 7, 9, 1, 4, 2, -7, 9 ,0};
		int[] result=bst.sort(testInt);
		
		System.out.println("Binary search:"+Arrays.toString(result));

	}
	
	@Override
	public int[] sort(int[] arr) {
		BSTSort sortBST1 = new BSTSort();	
		for (int i=0;i<arr.length;i++){
			sortBST1.insert(arr[i]);
		}
		sortBST1.printTree();
		
		int[] treeElements=new int[arrayList.size()];
		for (int i=0;i<arrayList.size();i++)
		{
			treeElements[i]=arrayList.get(i);
		}
		return treeElements;
	}
